...
@php
    ...
    $isQuickFilterable = in_array($name, ['certification.name', 'methodForm.name']);
    $quickFilterKey = $name === 'certification.name' ? 'certification' : ($name === 'methodForm.name' ? 'method' : null);
@endphp
...
>
    ...
    >
        <span
            class="fi-ta-header-cell-label text-sm font-semibold text-gray-950 dark:text-white flex items-center gap-1"
        >
            <span>{{ $slot }}</span>

            @if ($isQuickFilterable && $quickFilterKey)
                <button
                    type="button"
                    ...
                    x-on:click.stop.window=""
                    x-on:click.stop="window.dispatchEvent(new CustomEvent('fi-open-table-filters', { detail: { for: '{{ $quickFilterKey }}' } }))"
                >
